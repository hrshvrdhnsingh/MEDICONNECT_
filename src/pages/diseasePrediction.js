...
export default function SymptomCheckbox() {
    ...
    useEffect(() => {
        setStartupLoading(true); // For the free-server warning
        fetch(process.env.NEXT_PUBLIC_DISEASE_PREDICTION_API_URL) // All while warm up the server for prediction
            .catch((err) => console.error("API warmup error:", err))
            .finally(() => setStartupLoading(false));
    }, []);

    useEffect(() => { // For the timer in the countdown
    ...
    const handlePredict = async () => {
        ...
        try {
            ...
            // Actually calling the predicted diseases
    ...
    return (
        ...
        <div className={styles.prediction_bg}>
            {startupLoading && (
                <div style={{
                        backgroundColor: "#182f5d", minHeight: "100vh", width: "100vw", display: "flex",
                        alignItems: "center", justifyContent: "center", position: "fixed", top: 0, left: 0,
                        zIndex: 120, opacity: 0.98,
                    }}
                >
            ...
            <div className="flex justify-center items-center lg:mt-16 mt-16 flex-col">
                ...
                {/* Mapping the disease category with the symptoms for it to be selected */}
