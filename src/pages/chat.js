...
// Runs on the server before loading the chat page so that the page come pre-populated. 
// That means no spinner on the client-side => better SEO
// The firebase-admin logic is never exposed to the browser => Security
export async function getServerSideProps(context) {
    const cookieLib = require("cookie");
    const cookies = cookieLib.parse(context.req.headers.cookie || ""); // Cookie header into JS object
    const userType = cookies.userType; // Doctor or User
    ...
    // If the current user is a doctor, get all the patients for the current doctor
    if (userType === "doctor") {
        const res = await fetch(
            `${process.env.NEXT_PUBLIC_BASE_URL || "http://localhost:3000"}/api/getPatientsForDoctor`,
            {
    ...
    // Else get all the doctors for the patient to consult
    else if (userType) {
        const res = await fetch(
            `${process.env.NEXT_PUBLIC_BASE_URL || "http://localhost:3000"}/api/getDoctors`,
            {
                headers: { Cookie: context.req.headers.cookie || "" },
            }
        );
    ...
    return { // Pass the list to the React Component
        props: {
            initialList: list,
        },
    };
...
const Chat = ({ initialList = [] }) => {
    ...
    // Scroll to the newest message on new message arrival
    useEffect(() => {
    ...
    useEffect(() => {
        ...
        // The doctor vs patient logic determination
        ...
        // Tell the server who is in the conversation
        socket.emit("joinRoom", { user_uid: patient_uid, doctor_uid });

        // When the server sends over the previous messages, store them
        socket.on("previousMessages", (previousMessages) => {
            setMessages(previousMessages || []);
        });

        // On getting a new message, add that to the list
        socket.on("receiveMessage", (newMessage) => {
            setMessages((prevMessages) => [...prevMessages, newMessage]);
        });

        // Cleanup the event listeners when switched to another conversation
        return () => {
    ...
    const sendMessage = () => {
        ...
        // Re-derive the uids, just in case
        if (initialUserType === "doctor") {
            doctor_uid = initialUserUid;
            patient_uid = selected.uid;
        } 
        ...
        // Emits an event when sent a new message with the metadata.
        socket.emit("sendMessage", {
            sender: initialUserType || "user", message,
            user_uid: patient_uid, doctor_uid,
        });
    ...
    // For a doctor, this dispays either the fullname || name || email. But for patient, only the full name.
    ...
    return (
        <div className="flex h-screen w-screen overflow-hidden">
            {/* List (Doctors or Patients) in the sidebar */}
            ...
            <div className="flex flex-col bg-white w-10/12">
                <div className="border-b-1 flex justify-between items-center gap-2 p-4 border-[#e5e7eb] font-medium text-xl bg-[#f1f5f9]">
                    {/* Display the doctor with his specialization Or the patients name */}
                    {selected ? (
                        ...
                        <div className="font-medium text-xl">
                            Select a {initialUserType === "doctor" ? "patient" : "doctor"} to chat
                        </div>
                    )}
                    {/* Exclusive navbar for the chat page only */}
                    <ChatNavbar className="border border-black" />
                </div>
                {/* Diplaying of the messages */}
                <div className="flex-1  overflow-y-auto p-3 bg-[#f8fafc]">
                    {messages.length > 0 ? (
                        ...
                        <div className="text-[#64748b] mt-8">
                            Select a {initialUserType === "doctor" ? "patient" : "doctor"} to start
                            chatting.
                        </div>
                    )}
                </div>
